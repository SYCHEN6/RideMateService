...
public class DefaultFileParser implements FileParser {
    ...
    public ParsedDocument parse(MultipartFile file) throws Exception {
        logger.info("开始解析未知类型文件: {}", file.getOriginalFilename());
        ParsedDocument parsedDocument = new ParsedDocument();
        parsedDocument.setFileType(ParsedDocument.FileType.UNKNOWN);
        ...
        try (InputStream stream = file.getInputStream()) {
            ...
            logger.info("未知类型文件解析成功: {}, 内容长度: {}", file.getOriginalFilename(), handler.toString().length());
        } catch (Exception e) {
            logger.error("解析未知类型文件失败: {}", file.getOriginalFilename(), e);
            throw e;
        }
    ...
    public ParsedDocument.FileType getSupportedFileType() {
        return ParsedDocument.FileType.UNKNOWN;
    }
}
