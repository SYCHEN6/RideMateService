...
public class ExcelFileParser implements FileParser {
    ...
    public ParsedDocument parse(MultipartFile file) throws Exception {
        logger.info("开始解析Excel文件: {}", file.getOriginalFilename());
        ParsedDocument parsedDocument = new ParsedDocument();
        parsedDocument.setFileType(ParsedDocument.FileType.EXCEL);
        ...
        try (InputStream stream = file.getInputStream()) {
            ...
            logger.info("Excel文件解析成功: {}, 内容长度: {}", file.getOriginalFilename(), handler.toString().length());

            // 提取表格（后续实现）
        } catch (Exception e) {
            logger.error("解析Excel文件失败: {}", file.getOriginalFilename(), e);
            throw e;
        }
    ...
    public ParsedDocument.FileType getSupportedFileType() {
        return ParsedDocument.FileType.EXCEL;
    }
}
