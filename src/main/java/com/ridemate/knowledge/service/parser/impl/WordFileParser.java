...
public class WordFileParser implements FileParser {
    ...
    public ParsedDocument parse(MultipartFile file) throws Exception {
        logger.info("开始解析Word文件: {}", file.getOriginalFilename());
        ParsedDocument parsedDocument = new ParsedDocument();
        parsedDocument.setFileType(ParsedDocument.FileType.WORD);
        ...
        try (InputStream stream = file.getInputStream()) {
            ...
            logger.info("Word文件解析成功: {}, 内容长度: {}", file.getOriginalFilename(), handler.toString().length());

            // 提取图片和表格（后续实现）
        } catch (Exception e) {
            logger.error("解析Word文件失败: {}", file.getOriginalFilename(), e);
            throw e;
        }
    ...
    public ParsedDocument.FileType getSupportedFileType() {
        return ParsedDocument.FileType.WORD;
    }
}
